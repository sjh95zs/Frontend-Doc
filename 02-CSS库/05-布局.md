...
>
> 通过设置父元素的 display 属性为 display：flex | inline-flex；其子元素便有了伸缩性，即使在子元素的宽高不确定的情况下，也能通过设置相关 css 属性来决定子元素的对齐方式、所占比例和空间分布
...
3. flex-flow：它是 flex-direction 和 flex-wrap 的简写，默认值是 row no-wrap
...
6. align-content：定义了多行在交叉轴方向的对齐方式。项目排列只有一行时，该属性不起作用
