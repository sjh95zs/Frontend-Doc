...
> `$("selector");`：返回数组或空数组，每个元素都是一个引用了 DOM 节点的对象。数组的顺序按照 HTML 结构，而不是选择的顺序
...
`$("sel1,sel2,sel3,...,selN");`：结果合并返回一个数组
...
1. 元素
   ...
   - `:input、:text、:password、:radio、:checkbox、:button、:file`等等

2. 状态

   - `:enabled、:disabled、:checked、:selected` 等等

## 查找、过滤的方法

- 后代：`find("selector");`
- 子代：`children("selector");`
- 唯一父亲：`儿子节点.parent();`
- 兄弟：`节点.next();`或`节点.prev();`
- 获取返回数组中的第 n 个 元素：`节点.eq(n);`
